...
from glass_post.detail.ltm.neur_br.utils.image_io import *
from glass_post.detail.ltm.neur_br.utils.imresize import imresize
...
def get_downsampled(image, downsample_factors):
    ...
    for i in downsample_factors:
        downsampler = Downsampler(
            n_planes=image_torch.shape[1], factor=i, kernel_type="lanczos2", phase=0.5, preserve_size=True
        ).cuda()
...
def get_imresize_downsampled(image, downsampling_factor, downsampling_number):
    ...
    for i in range(1, downsampling_number + 1):
        im = np.clip(
            imresize(
                image.transpose(1, 2, 0), scale_factor=(1 - (downsampling_factor * downsampling_number))
            ).transpose(2, 0, 1),
            0,
            1,
        )
