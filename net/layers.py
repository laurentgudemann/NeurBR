...
import numpy as np
from glass_post.detail.ltm.neur_br.net.downsampler import Downsampler
from glass_post.detail.ltm.neur_br.utils.image_io import torch_to_np
...
class Ratio(nn.Module):
    def __init__(self):
        ...
        self.multp = torch.autograd.Variable(torch.tensor(np.random.uniform(0, 1)), requires_grad=True).type(
...
class VectorRatio(nn.Module):
    def __init__(self, frames_number):
        ...
        self.multp = torch.autograd.Variable(
            torch.tensor(np.random.uniform(0, 1, frames_number)).reshape([frames_number, 1, 1, 1])
...
class CovarianceLayer(nn.Module):
    ...
    def forward(self, x, y):
        return F.conv2d(
            (F.conv2d(x, self.ones_mask) - F.conv2d(x, self.mean_mask))
            * (F.conv2d(y, self.ones_mask) - F.conv2d(y, self.mean_mask)),
            self.mean_mask,
        )
